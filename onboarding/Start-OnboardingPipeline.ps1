...
if (-not $resolvedContentRepoUrl -and $config.deployment) {
    $resolvedContentRepoUrl = $config.deployment.contentRepoUrl
}
if (-not $resolvedContentRepoUrl -and $config.deployment) {
    $resolvedContentRepoUrl = $config.deployment.repositoryUrl
}
if (-not $resolvedContentRepoUrl) {
    throw "ContentRepoUrl is required. Provide -ContentRepoUrl or set contentRepo.url in the customer config."
}

$resolvedContentRepoBranch = $ContentRepoBranch
if (($resolvedContentRepoBranch -eq '') -and $config.contentRepo) {
    $resolvedContentRepoBranch = $config.contentRepo.branch
}
if (-not $resolvedContentRepoBranch) {
    $resolvedContentRepoBranch = 'main'
}
...
$sentinelContentArgs = @(
    '-CustomerConfigPath', $CustomerConfigPath,
    '-ContentRepoUrl', $resolvedContentRepoUrl,
    '-BranchName', $resolvedContentRepoBranch,
    '-EvidenceOutputPath', $EvidenceOutputPath
)
...
Write-Host "Content repo: $resolvedContentRepoUrl ($resolvedContentRepoBranch)" -ForegroundColor Gray
...
Write-Evidence @{
    ...
    managedByTenantId  = $ManagedByTenantId
    contentRepoUrl     = $resolvedContentRepoUrl
    contentRepoBranch  = $resolvedContentRepoBranch
    ...
    testRequired       = @(
        ...
        'TEST_REQUIRED: ContentRepoUrl must resolve to the private source-of-truth repository for Sentinel content'
    )
