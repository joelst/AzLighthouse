...
.PARAMETER PolicyTemplateDisplayName
    Display name for a newly created template. Default: "RSOC Managed Services Governance Policy".
...
param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$PolicyTemplateDisplayName = 'RSOC Managed Services Governance Policy',
...
if (-not $pendingInvitation) {
    ...
    Write-Host "  WAITING FOR CUSTOMER ACTION  No valid invitation found        " -ForegroundColor Yellow
    Write-Host "================================================================" -ForegroundColor Yellow
...
if (-not $PolicyTemplateId) {
    ...
    $rsocTemplate = $templates.value |
        Where-Object { $_.displayName -like '*RSOC*' -or $_.displayName -like '*ManagedServices*' } |
        Select-Object -First 1
    ...
    if ($rsocTemplate -and -not $CreateTemplate) {
        ...
        $templateBody = @{
            ...
            description                            = 'RSOC delegated Sentinel/Defender admin policy. Groups from AzLighthouse/lighthouse/lighthouse-offer1.json.'
...
Write-Host "  MANUAL ACTION REQUIRED  Customer must approve request (STEP 3)" -ForegroundColor Magenta
Write-Host "================================================================" -ForegroundColor Magenta
Write-Host ""
...
Write-Evidence @{
    ...
    testRequired             = @(
        "API is Graph beta - verify endpoints before production",
        "RSOC tenant must have governance invitations enabled before customer sends invitation",
