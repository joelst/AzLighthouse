...
.DESCRIPTION
    Loads a manifest JSON array of {name, type, armId, content} objects, checks each deployed item
    against ARM APIs for Sentinel analytics rules, workbooks, and automation rules, then reports
    missing or modified items. With -Remediate, redeploys diverged/missing content.
    TEST_REQUIRED: Manifest format must be defined; Sentinel content APIs vary by type;
    workbooks use a different base path.
#>
...
if ($WhatIfMode) {
    ...
    Write-Evidence @{
        ...
        testRequired      = @(
            'TEST_REQUIRED: Manifest format must be validated before use',
            'TEST_REQUIRED: Workbooks use a different base path - verify API versions',
            'TEST_REQUIRED: analyticsRule vs automationRule API paths differ'
        )
...
Write-Evidence @{
    ...
    testRequired      = @(
        ...
        'TEST_REQUIRED: Drift detection uses property comparison - verify schema alignment'
    )
    driftReport       = $driftReport
}
