...
# TEST_REQUIRED: Confirm exact ARM parameter names from  names below are best-guess from common Sentinel All-in-One patternstemplate 
$armParams = @{
    workspaceName     = $workspaceName
    location          = $region
    enableSentinel    = $true
...
try {
    ...
    if ($sentinelSolution) {
        ...
        Write-Host "  [WARN] Sentinel solution not found in solution  may require portal check." -ForegroundColor Yellowlist 
    ...
    try {
        ...
        if ($sentinelResource) {
            ...
            Write-Host "  [WARN] Sentinel solution not  check portal manually." -ForegroundColor Yellowconfirmed 
...
$manualConnectorAction = @{
    ...
    connectors = @(
        'Microsoft 365 Defender ( requires M365 GA Admin consent',MicrosoftThreatProtection) 
        'Azure Active Directory ( requires AAD GA Admin',AAD) 
        'Office  requires Exchange/SharePoint admin',365 
        'Microsoft Defender for Cloud  requires MCAS admin'Apps 
    )
...
Write-Host ""
" -ForegroundColor YellowWrite-Host "
Write-         MANUAL_ACTION  Customer GA " -ForegroundColor YellowAdmin            REQUIRED Host "
#Write-Host "
" -ForegroundColor Yellow
Write-  The following connectors require customer GA Admin to " -ForegroundColor Yellowenable: Host "
 Microsoft 365 " -ForegroundColor YellowDefender                                    Write-   Host "
 Azure Active " -ForegroundColor YellowDirectory                                    Write-   Host "
 Office " -ForegroundColor Yellow365                                                Write-   Host "
 Microsoft Defender for Cloud " -ForegroundColor YellowApps                        Write-   Host "
Write-  Portal: Sentinel > Data connectors > [connector] > " -ForegroundColor YellowConnect  Host "
 -ForegroundColor YellowWrite-Host "
