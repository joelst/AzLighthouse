...
try {
    ...
    if ($statusCode -eq 404) {
        Write-Host "  Repositories endpoint returned  Sentinel Repositories feature may not be enabled on this workspace." -ForegroundColor Yellow404 
...
if ($repoExists) {
    ...
    try {
        ...
        if ($errMsg -match 'auth|credential|unauthorized|permission' -or $statusCode -in @(401, 403)) {
            Write-Host ""
" -ForegroundColor Magenta            Write-Host "  
...
try {
    ...
    $statusCode = $_.Exception.Response?.StatusCode.value__ ?? 0
    if ($statusCode -eq 404) {
        Write-Host "  [INFO] Sync endpoint returned  sync may not be available or connection is still initializing." -ForegroundColor Yellow404 
...
Write-Evidence @{
    ...
    manualActions     = @(
        ...
        "Maximum 5 repositories per  validate before adding",workspace 
