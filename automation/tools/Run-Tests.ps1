...
<#
.SYNOPSIS
    Test runner script for Update-AppRegistrationCredential.ps1
.DESCRIPTION
...
.PARAMETER ConfigPath
    Path to the Pester configuration file. Defaults to ..\Tests\PesterConfig.psd1
.PARAMETER TestPath
    Path to the test file. Defaults to ..\Tests\Update-AppRegistrationCredential.ps1.Tests.ps1
...
param(
  ...
  [string]$ConfigPath = '..\Tests\PesterConfig.psd1',

  [Parameter()]
  [ValidateScript({ Test-Path $_ -PathType Leaf })]
  [string]$TestPath = '..\Tests\Update-AppRegistrationCredential.ps1.Tests.ps1',
...
try {
  ...
  if (Test-Path $ConfigPath) {
    ...
    $Config = Import-PowerShellDataFile -Path $ResolvedConfigPath.Path
        ...
        $Config.Run.Path |
          ForEach-Object {
            ...
            if (-not [System.IO.Path]::IsPathRooted($PathToResolve)) {
              $PathToResolve = Join-Path $ConfigDirectory $PathToResolve
            }

            Resolve-Path $PathToResolve -ErrorAction SilentlyContinue |
    ...
    if ($CodeCoverage) {
      ...
      $PesterConfig.CodeCoverage.Path = '..\Update-AppRegistrationCredential.ps1'
