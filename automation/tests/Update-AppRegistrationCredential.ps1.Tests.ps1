...
BeforeAll {
  ...
  # Extract only function definitions so tests avoid the script's production startup path.
  $scriptContent = Get-Content $scriptPath -Raw
  $helperFunctions = [regex]::Match(
    $scriptContent,
    '(?s)\s*function Assert-ValidGuid \{.*?(?=# Set variables from automation account variables if not already set\.)'
  ).Value
  $runbookFunctions = [regex]::Match(
    $scriptContent,
    '(?s)\s*function New-SecretNotification \{.*?(?=# Ensures you do not inherit an AzContext in your runbook)'
  ).Value

  if ([string]::IsNullOrWhiteSpace($helperFunctions) -or [string]::IsNullOrWhiteSpace($runbookFunctions)) {
    throw 'Unable to extract functions from Update-AppRegistrationCredential.ps1 for testing.'
  }

  $functionsOnly = @(
    $helperFunctions.Trim()
    $runbookFunctions.Trim()
